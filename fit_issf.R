  ...
  stp.var <- foreach(i = 1:nrow(data)) %do%
    {
      ...
      env$HR <- NA
      env$HR <- values(terra::distance(env$ele, median_pt)) / 1000
...
formula_df <- data.frame(
  formula = c(
    ...
    "case_ ~ log(sl_) + cos(ta_) + HR_end + ndvi_end + strata(step_id_)"
  ),
  name = c("null_model_lin", "ndvi")
)
    ...
    foreach(h = 1:10, .combine = "rbind") %do%
      {
        res <- simulate_movement(
          deer_mvt[i, ],
          env_rasters[[1]],
          ndvi_rasters[[paste("ndvi", deer_mvt[i, ]$year, sep = "_")]],
          model_sim,
          formula_df[1, 1]
        )
...
sim_res_ndvi <- foreach(i = 1:10) %do%
  {

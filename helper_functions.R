...
make_random_pt_extraction <- function(
  ...
  random.stp <- purrr::map(1:nrow(data), function(i) {
    print(i)
...
extract_step_variables <- function(
  ...
  stp.var <- purrr::map(1:nrow(data), function(i) {
    print(i)
    ...
    env$HR <- NA
    env$HR <- terra::values(terra::distance(env, median_pt)) / 1000
    ...
    # Process step data
    data_ssf <- data_row[[random_col]][[1]] |>
...
run_simulations <- function(
  ...
  furrr::future_map(1:n_deer, function(i) {
    crop_i <- deer_crops[[i]]
    deer_i <- deer_rows[[i]]

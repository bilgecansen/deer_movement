...
#' itself -- amt's (its own Jacobian, normalisation, rasterisation) against ours.
#' A shared simple cov_fun is used rather than the two production ones, precisely
#' so that a disagreement can only come from the kernel maths.
#'
#' Usage: Rscript scripts/checks/check_differential_gam.R [n_deer] [n_steps]

suppressPackageStartupMessages({
  library(mgcv); library(amt); library(terra)
  library(tidyverse); library(sf); library(survival)
})
...
all_keys <- all_keys[file.exists(sprintf("results/gam/results_gam_%s.rds", all_keys))]
...
for (key in keys) {
  p <- strsplit(key, "_")[[1]]
  id <- p[1]; season <- p[2]; year <- as.integer(p[3])
  ...
  attr(d, "sl_") <- sl_distr; attr(d, "ta_") <- ta_distr; attr(d, "crs") <- 6610
  class(d) <- unique(c("random_steps", "steps_xyt", "steps_xy", class(d)))

  # ---- (a) coefficients ----------------------------------------------------
  iss <- tryCatch(
    amt::fit_issf(d, stats::as.formula(paste("case_ ~", RHS, "+ strata(step_id_)")),
                  model = TRUE),
    error = function(e) NULL)
  gd <- prepare_gam_data(d)
  gm <- tryCatch(
    mgcv::gam(stats::as.formula(paste("cbind(times, stratum) ~", RHS)),
              data = gd, family = mgcv::cox.ph(), weights = obs, method = "REML"),
    error = function(e) NULL)

  if (is.null(iss) || is.null(gm)) {
    check(sprintf("%s: both models fit", key), NA, "a fit failed"); next
  }
  ci <- coef(iss$model)[TERMS]; cg <- coef(gm)[TERMS]
  ...
  dens <- function(k, at) { e <- terra::extract(k, at); as.numeric(e[1, ncol(e)]) }
  ...
  check(sprintf("%s: per-step probabilities agree", key), max(abs(dd)) < 1e-6,
        sprintf("max |diff| = %.2e, cor = %.8f", max(abs(dd)), cor(lp$issf, lp$gam)))

  summary_rows[[key]] <- tibble(key = key, season = season, n_strata = max(gd$stratum),

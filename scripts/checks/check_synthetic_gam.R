...
#' only -- the case/control design is built directly rather than through
#' the wrangle).
#'
...
#'   one -- the movement corrections, which must be ~0 because the tracks were
...
#'         redistribution kernel is normalised over the disc and evaluated at
#'         the observed endpoint, exactly as onestep_logscore*() does. Two
...
#' touches the planar redistribution kernel -- see
#' scripts/checks/synth_helpers.R for why that independence is the whole point.
#'
...
fit_gam_synth <- function(d, rhs) {
  d$times <- 1
  d$stratum <- as.integer(factor(d$step_id_))
  d$obs <- as.integer(d$case_)
...
if (part %in% c("1", "both")) {
  ...
  truth <- f_true(grid)
  truth <- truth - mean(truth)   # smooths are sum-to-zero
  ...
  reps <- purrr::map_dfr(seq_len(N_REPS), function(rep) {
    ...
    tibble(rep = rep,
           ...
           edf = sum(g$edf),
           b_sl = cf[["sl_"]], b_logsl = cf[["log(sl_)"]],
           b_costa = cf[["cos(ta_)"]])
  })
  ...
  check("fitted peak within 0.10 of the true peak",
        ...
        sprintf("peaks = %s (truth %.2f)",
                paste(round(reps$peak, 3), collapse = ", "), PEAK))
  ...
  for (nm in c("b_sl", "b_logsl", "b_costa")) {
    ...
    check(sprintf("movement correction %s ~ 0 (truth: no modification)", nm),
          ...
          sprintf("mean %.4f (sd %.4f) over %d reps",
...
if (part %in% c("2", "both")) {
  ...
  d <- synth_design(trk, land, shape = SHAPE, scale = SCALE,
                    kappa = KAPPA, seed = 8)
  ...
  # raster, normalise it, read the density at the observed endpoint.
  ...
  set.seed(11)
  idx <- sort(sample(nrow(stp), n_eval))

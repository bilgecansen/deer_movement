...
#' `filters/amt/filter_combined_amt.rds` (which carries a `dropped_at` column
...
annotated <- readRDS("filters/amt/filter_combined_amt_test.rds")
...
plot_deer_models <- function(key, model_info, title, sim_alpha = 0.25) {
  ...
  sim_file <- sprintf("sims/amt/sims_amt_%s.rds", key)

...
# scripts/amt/run_logscore_amt.R on each. By default, skip deer whose
# log-score
...
#
# Usage:
#   bash scripts/amt/run_logscore_all.sh              # resumable (default)
#   bash scripts/amt/run_logscore_all.sh --overwrite  # reprocess all
...
for f in results/amt/results_amt_*.rds; do
  ...
  out_path="filters/amt/logscore_amt_${key}.rds"
  ...
  echo "[run]  $key"
  if Rscript scripts/amt/run_logscore_amt.R "$id" "$season" "$year"; then
    n_done=$((n_done + 1))
  else
    rc=$?

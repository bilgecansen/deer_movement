#!/usr/bin/env bash
# Iterate every deer with test-year simulations in sims/sims_test_*.rds and
# run scripts/run_udoverlap_test.R on each. Driving off the test sims (rather
...
#
# Usage:
#   bash scripts/run_udoverlap_all_test.sh              # resumable (default)
#   bash scripts/run_udoverlap_all_test.sh --overwrite  # reprocess all
...
for f in sims/sims_test_*.rds; do
  base=$(basename "$f" .rds)
  key=${base#sims_test_}
  IFS='_' read -r id season year <<< "$key"

  out_path="filters/udoverlap_test_${key}.rds"
  ...
  echo "[run]  $key"
  if Rscript scripts/run_udoverlap_test.R "$id" "$season" "$year"; then
    n_done=$((n_done + 1))
  else
    rc=$?
